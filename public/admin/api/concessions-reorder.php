...
/**
 * JSON endpoint for concessions.php's drag-to-reorder UI.
 * Accepts {"order":[{"id":..,"sort_order":..}, ...]} and persists the new
 * sort_order for every listed concession item in a single transaction.
 *
 * This is a JSON POST, not a form submit, so the CSRF token travels via the
 * X-CSRF-Token request header (same convention as the movies-reorder
 * endpoint and public/api/cart.php) instead of a hidden form field, but is
 * validated through the exact same AdminAuth::validateCsrf() every other
 * admin POST handler uses — CSRF protection is not skipped just because the
 * payload is JSON.
 */
...
if (count($ids) === 0) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'No items to reorder.']);
    exit;
}
...
try {
    ...
    $stmt = $db->prepare('UPDATE concessions SET sort_order = :sort_order WHERE id = :id');
    ...
    error_log('concessions-reorder save failed: ' . $e->getMessage());
